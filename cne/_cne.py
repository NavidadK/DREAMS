...
class Lin_Decoder(torch.nn.Module):
    ...
    def __init__(self, X, n, n_features, emb_dim, init=None, init_weights=None):
        ...
        if init_weights is not None:
            ...
            #self.decoder.weight = torch.nn.Parameter(torch.tensor(init_weights, dtype=torch.float32))
            #self.decoder.bias = torch.nn.Parameter(torch.zeros(n_features))
            # print(self.decoder.weight[:5])
        else:
